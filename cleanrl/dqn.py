...
class QNetwork(nn.Module):
    def __init__(self, env, risk_size=0):
        ...
        self.network = nn.Sequential(
            nn.Linear(np.array(env.observation_spec()["board"].shape).prod()+risk_size, 120),
            nn.ReLU(),
            nn.Linear(120, 84),
            nn.ReLU(),
            nn.Linear(84, 4),
...
"""
    ...
    # assert isinstance(envs.singaction_space, gym.spaces.Discrete), "only discrete action space is supported"
    ...
    if args.use_risk:
        risk_model = risk_model_class[args.model_type][args.risk_type](obs_size=np.array(envs.observation_spec()["board"].shape).prod(), batch_norm=True, out_size=risk_size)
    ...
    if args.fine_tune_risk != "None" and args.use_risk:
        if args.rb_type == "balanced":
            ...
            risk_rb = ReplayBuffer(buffer_size=args.total_timesteps)
                          #, observation_space=envs.single_observation_space, action_space=envs.action_space)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if random.random() < epsilon and args.mode == "train":
            ...
            obs_in = torch.Tensor(obs["board"]).reshape(1, -1).to(device)
            with torch.no_grad():
        ...
        if done:
            print(envs.environment_data['safety'])
